...
# Cleanly prompt user input
folder_subpath = input("Enter subfolder path inside Google Drive 'raspi_logs' (e.g., 20250518_233829): ").strip().lstrip('/')
imu_filename = input("Enter IMU data filename (e.g., imu_data.txt): ").strip()
lidar_filename = input("Enter LiDAR data filename (e.g., lidar_data.txt): ").strip()
...
# Local paths
IMU_FILE = os.path.join(LOCAL_DIR, imu_filename)
LIDAR_FILE = os.path.join(LOCAL_DIR, lidar_filename)
...
if result_imu.returncode != 0 or result_lidar.returncode != 0:
    print("[Error] One or both files could not be downloaded. Check folder and filenames.")
    exit(1)
